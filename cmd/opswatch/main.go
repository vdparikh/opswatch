...
func notifyAlerts(alerts []domain.Alert) {
	for _, alert := range alerts {
		...
		message := notificationMessage(alert)
		_ = exec.Command(
			"osascript",
			"-e",
...
func notificationMessage(alert domain.Alert) string {
	observed := ""
	intent := ""

	for _, evidence := range alert.Evidence {
		switch {
		case strings.HasPrefix(evidence, "observed: "):
			observed = strings.TrimPrefix(evidence, "observed: ")
		case strings.HasPrefix(evidence, "command: "):
			observed = strings.TrimPrefix(evidence, "command: ")
		case strings.HasPrefix(evidence, "intent: "):
			intent = strings.TrimPrefix(evidence, "intent: ")
		}
	}

	switch {
	case observed != "" && intent != "":
		return "Observed: " + observed + " | Intent: " + intent
	case observed != "":
		return "Observed: " + observed
	case intent != "":
		return "Intent: " + intent
	case len(alert.Evidence) > 0:
		return alert.Evidence[0]
	default:
